...
InternalHandler ml = new InternalHandler();
  ...
  */
   ...
   * If false, only text will be painted with marked color.
   */
  ...
  public void setPairsPainter(boolean paint, int style){
    ...
    if (!paint) highlightStyle = HLS_NONE;
  }
  ...
  public boolean selectPair(){
    ...
    baseEditor.searchGlobalPair(cp);
    if (cp.end == null) return false;
    if (cp.topPosition) text.setSelection(text.getOffsetAtLine(cp.sline)+cp.start.start,
  ...
  public boolean selectContentPair(){
    ...
    baseEditor.searchGlobalPair(cp);
    if (cp.end == null) return false;
    if(cp.topPosition) text.setSelection(text.getOffsetAtLine(cp.sline)+cp.start.end,
                                        text.getOffsetAtLine(cp.eline)+cp.end.start);
    else text.setSelection(text.getOffsetAtLine(cp.eline)+cp.end.end,
  ...
  void drawLine(int lno) {
    ...
    if (text.getLineCount() > lno+1)
      ...
      height = text.getLocationAtOffset(text.getCharCount()).y + text.getLineHeight();
    int width = text.getClientArea().width + text.getHorizontalPixel();
    text.redraw(0, y, width, height, false);
  }
  ...
  class InternalHandler implements VerifyListener, ExtendedModifyListener, ControlListener, TextChangeListener,
    ...
    public void lineGetStyle(LineStyleEvent e){
      ...
      for(int idx = 0;idx < lrarr.length; idx++){
        ...
        StyleRange sr = new StyleRange(start, end,
                            cm.getColor(rdef.bfore, rdef.fore), cm.getColor(rdef.bback, rdef.back),
                            rdef.style);
    ...
    public void lineGetBackground(LineBackgroundEvent e){
      ...
      if (horzCrossColor != null && horzCross && lineHighlighting &&
        ...
        e.lineBackground = cm.getColor(((StyledRegion)horzCrossColor).bback, ((StyledRegion)horzCrossColor).back);
        return;
      }
