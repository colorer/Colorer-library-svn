...
void ConsoleTools::listTypes(bool names, bool load){
  Writer *writer = null;
...
class TokenHandler : public RegionHandler
{
  ...
  Vector<int> regionStackPos;
  Vector<const Region*> regionStack;
...
public:
  ...
  void writeMixed(int x, int len){
    String *line = lineStore->getLine(y);
    int tox = x+len;
    while(x <= tox){
      int i;
      for(i = regionStack.size()-1; i >= 0; i--){
        const Region *region = regionStack.elementAt(i);
        int pos = regionStackPos.elementAt(i);
        if (pos == x){
          regionStack.removeElementAt(i);
          regionStackPos.removeElementAt(i);
  ...
  void startParsing(int lno){
    x = y = 0;
    regionStackPos.clear();
    regionStack.clear();
  }
  void endParsing(int lno){
    writeTill(lno, lineStore->getLine(lno)->length());
  }
  ...
  void addRegion(int lno, String *line, int sx, int ex, const Region *region){
    ...
    regionStackPos.addElement(ex);
    regionStack.addElement(region);
  }
