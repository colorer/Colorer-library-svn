...
void HRCParserImpl::parseHRC(InputSource *is)
{
  ...
  if (globalUpdateStarted){
    CLR_TRACE("HRC", "parseHRC: updateLinks");
    updateLinks();
    updateStarted = false;
  }
}
...
{
  Vector<SchemeImpl*> schemesToExpand;
  ...
  while(structureChanged){
    ...
    for(SchemeImpl *scheme = schemeHash.enumerate(); scheme != null; scheme = schemeHash.next(), num_schemes++){
      if (!scheme->fileType->loadDone) continue;
      FileTypeImpl *old_parseType = parseType;
      parseType = scheme->fileType;

      CLR_TRACE("HRC", "updateLinks: %s", scheme->getName()->getChars());

      bool marked_schemesToExpand = false;
      num_nodes += scheme->nodes.size();
        ...
        if (snode->type == SNT_INHERIT){
          for(int vti = 0; vti < snode->virtualEntryVector.size(); vti++){
            ...
            if (vt->virtScheme == null && vt->virtSchemeName != null){
              ...
              if (!vsn){
                if (errorHandler != null) errorHandler->error(StringBuffer("cannot virtualize scheme '")+vt->virtSchemeName+"' in scheme '"+scheme->schemeName+"'");
                failed_entry = true;
              }
              delete vsn;
            ...
            if (vt->substScheme == null && vt->substSchemeName != null){
              ...
              if (vsn) vt->substScheme = schemeHash.get(vsn);
                ...
                if (errorHandler != null) errorHandler->error(StringBuffer("cannot virtualize using subst-scheme scheme '")+vt->substSchemeName+"' in scheme '"+scheme->schemeName+"'");
                failed_entry = true;
              }
              delete vsn;
            ...
            if (failed_entry){
              snode->virtualEntryVector.removeElementAt(vti);
              vti--;
              delete vt;
            }
          }
          if (!marked_schemesToExpand){
            schemesToExpand.addElement(scheme);
            marked_schemesToExpand = true;
          }
        }
      }
  ...
  CLR_INFO("HRC", "updateLinks:stat:num_schemes=%d", num_schemes);
  CLR_INFO("HRC", "updateLinks:stat:num_nodes  =%d", num_nodes);

  for(int idx = 0; idx < schemesToExpand.size(); idx++)
  {
    SchemeImpl * scheme = schemesToExpand.elementAt(idx);
    if (!scheme->fileType->loadDone) continue;

    CLR_TRACE("HRC", "updateLinks:expandInheritance loop %s", scheme->getName()->getChars());

    for (int sni = 0; sni < scheme->nodes.size(); sni++){
      SchemeNode *snode = scheme->nodes.elementAt(sni);
      if (snode->type == SNT_INHERIT){
        /* Loadtime <inhert> expansion */
        sni = expandInheritance(&scheme->nodes, sni);
      }
    }
  }
...
int HRCParserImpl::expandInheritance(Vector<SchemeNode*> *nodes, int sni)
{
  SchemeNode *snode = nodes->elementAt(sni);

  assert(snode->type == SNT_INHERIT);

  CLR_TRACE("HRC", "expandInheritance node[%d/%d]: %s", sni, nodes->size(), snode->scheme ? snode->scheme->getName()->getChars():null);

  if (snode->scheme != null)
  {
    Vector<SchemeNode*> &foreignNodes = snode->scheme->nodes;

    for (int f_idx = 0; f_idx < foreignNodes.size(); f_idx++){
      SchemeNode *f_snode = foreignNodes.elementAt(f_idx);

      CLR_INFO("HRC", "expandInheritance:cloning node[%d], nodes->size=%d type:%s, schemeName=%s",
                f_idx, nodes->size(), schemeNodeTypeNames[f_snode->type], f_snode->schemeName ? f_snode->schemeName->getChars() : null);

      SchemeNode *cloned_snode = f_snode->clone();

      expandVirtualTables(cloned_snode, snode);

      nodes->insertElementAt(cloned_snode, sni);
      if (cloned_snode->type == SNT_INHERIT){
        sni = expandInheritance(nodes, sni);
      }
      sni++;
    }
  }

  CLR_TRACE("HRC", "expandInheritance:remove: %s, type=%s", nodes->elementAt(sni)->scheme ? nodes->elementAt(sni)->scheme->getName()->getChars() : null, schemeNodeTypeNames[nodes->elementAt(sni)->type]);

  /* Remove expanded <inherit> element */
  nodes->removeElementAt(sni);
  sni--;

  CLR_TRACE("HRC", "expandInheritance exit [sni=%d]", sni);

  return sni;
}

void HRCParserImpl::expandVirtualTables(SchemeNode *node, SchemeNode *parentNode)
{
  switch(node->type) {
    case SNT_SCHEME:
    case SNT_INHERIT:
      {
        Scheme *ret = node->scheme;
        int split_idx = -1;

        for(int idx = parentNode->virtualEntryVector.size()-1; idx >= 0; idx--){
          VirtualEntry *ve = parentNode->virtualEntryVector.elementAt(idx);
          if (ret == ve->virtScheme && ve->substScheme){
            ret = ve->substScheme;
            split_idx = idx;
          }
        }
        //TODO: Apply virtual list for the current <block>
        if (split_idx != -1){
          for (int idx = 0; idx <= split_idx; idx++){
            node->virtualEntryVector.insertElementAt( parentNode->virtualEntryVector.elementAt(idx), 0);
            /* Include all virtual entries from this block (looking for empty entry */
            if (idx == split_idx &&
                parentNode->virtualEntryVector.elementAt(idx)->virtScheme != null &&
                split_idx+1 < parentNode->virtualEntryVector.size())
            {
              split_idx++;
            }
          }
