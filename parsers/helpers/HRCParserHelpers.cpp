...
SchemeNode::~SchemeNode(){
  if ((type == SNT_RE || type == SNT_SCHEME) && !cloned){
    delete start;
    delete end;
  };
  if (type == SNT_KEYWORDS && !cloned){
    delete kwList;
    delete worddiv;
  };
  if (type == SNT_INHERIT || type == SNT_SCHEME){
...
SchemeNode *SchemeNode::clone(){
  SchemeNode *clone = new SchemeNode();
  clone->type = this->type;
  clone->cloned = true;
  if (this->schemeName != null){
    clone->schemeName = new SString(this->schemeName);
  }
  switch(this->type){
    case SNT_INHERIT:
      clone->scheme = this->scheme;
      for (int idx = 0; idx < virtualEntryVector.size(); idx++){
        clone->virtualEntryVector.addElement(virtualEntryVector.elementAt(idx));
      }
      break;
    case SNT_SCHEME:
      clone->scheme = this->scheme;
      ...
      clone->end = this->end;
      clone->innerRegion = this->innerRegion;
      clone->lowPriority = this->lowPriority;
      clone->lowContentPriority = this->lowContentPriority;
      for (int idx = 0; idx < virtualEntryVector.size(); idx++){
        clone->virtualEntryVector.addElement(virtualEntryVector.elementAt(idx));
      }
      break;
    case SNT_RE:
      ...
      break;
    case SNT_KEYWORDS:
      clone->kwList = this->kwList;
      clone->worddiv = this->worddiv;
      break;
  }
  return clone;
}
